pipeline {
    ...
    environment {
        ...
        SMTP_SERVER_PASS = credentials('3a621563-cc40-4215-9b25-9036a53c8c87')
        PORT = '465'
    ...
    stages {
        ...
        stage("Deploy") {
            steps {
                script {
                    dir('./k8s') {
                        withKubeCredentials(kubectlCredentials: [[caCertificate: '', clusterName: '', contextName: '', credentialsId: 'a27a529e-b287-41b0-9dd9-c3d6db66c456', namespace: '', serverUrl: '']]) {
                            try {
                                if (env.BRANCH_NAME == 'dev') {
